...
rem refresh Marianas
set OPENTRAINING_FOLDER=%VEAF_FOLDER%\Saved Games\DCS.missions\VEAF_OpenTraining-Marianas
echo OPENTRAINING_FOLDER=%OPENTRAINING_FOLDER%
FOR %%f IN ("%OPENTRAINING_FOLDER%\VEAF_OpenTraining_Marianas*.miz") DO (
	echo Refreshing OpenTraining for server %SERVERNAME% based on %%f
	call veaf-tools injectall --quiet "%%f" "%MISSION_FOLDER%\VEAF_OpenTraining_Marianas-${version}.miz" "%OPENTRAINING_FOLDER%\weatherAndTime\versions.json"
)

rem add the static missions
copy /y "%VEAF_FOLDER%\Saved Games\DCS.missions\_VEAF_Static.%SERVERNAME%\*.miz" "%MISSION_FOLDER%\"
copy /y "%VEAF_FOLDER%\Saved Games\DCS.missions\VEAF-Top-Gun-Academy\*.miz" "%MISSION_FOLDER%\"
